...
`test/fixtures/sample-project/`, so results are deterministic and don't
...
`test/run-search-code.mts` calls `searchCodeTool` directly:

```bash
npx tsx test/run-search-code.mts <query> <directory> [filePattern] [caseSensitive:true|false]
```
...
```text
test/fixtures/sample-project/src/store/index.ts
...
```bash
npx tsx test/run-search-code.mts "export" test/fixtures/sample-project .tsx
```

**Expected output:**

```text
test/fixtures/sample-project/src/components/Button.tsx
...
```bash
npx tsx test/run-search-code.mts "STORE" test/fixtures/sample-project "" true
```

**Expected output:**

```text
No matches found for "STORE" in test/fixtures/sample-project.
```

**3b — case-insensitive (default), should find matches:**

```bash
npx tsx test/run-search-code.mts "STORE" test/fixtures/sample-project
```

**Expected output:**

```text
test/fixtures/sample-project/src/hooks/useAuth.ts
...
```bash
npx tsx test/run-search-code.mts "nonexistent_symbol_xyz" test/fixtures/sample-project
```

**Expected output:**

```text
No matches found for "nonexistent_symbol_xyz" in test/fixtures/sample-project.
```
...
```bash
npx tsx test/run-search-code.mts "createStore" test/fixtures/sample-project/src .ts
```
...
```bash
npx tsx test/run-search-code.mts "MATCHME" test/fixtures/sample-project
```
...
npx tsx test/run-search-code.mts "ChatGroq" src

# Restrict to a specific file extension
npx tsx test/run-search-code.mts "ChatGroq" src .ts

# Case-sensitive search
npx tsx test/run-search-code.mts "SystemMessage" src "" true

# Search within a specific subdirectory only
npx tsx test/run-search-code.mts "tool" src/tools
```
...
The fixtures in `test/fixtures/` are static and version-controlled test
